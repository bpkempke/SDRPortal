...
int main(int argc,char *argv[]){
  ...
  if((vp = create_viterbi27_port(framebits)) == NULL){
    printf("create_viterbi27_port failed\n");
    exit(1);
  }
  if(ebn0 != -100){
	...
	sr = (sr << 1) | bit;
	bits[i/8] = sr & 0xff;
	symbols[2*i+0] = addnoise(parity(sr & V27POLYB),gain,Gain,127.5,255);
	symbols[2*i+1] = addnoise(parity(sr & V27POLYA) ^ 1,gain,Gain,127.5,255);
      }
